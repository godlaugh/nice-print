...
import { InsertUser, users, conversions, slides, InsertConversion } from "../drizzle/schema";
...
export async function createConversion(data: InsertConversion) {
  ...
  const [result] = await db.insert(conversions).values(data).$returningId();
  return result.id;
}
...
export async function deleteConversion(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(slides).where(eq(slides.conversionId, id));
...
export async function getSlidesByConversion(conversionId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(slides).where(eq(slides.conversionId, conversionId));
}
